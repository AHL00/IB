...
/// Input a string from the user and validate it using a closure.
/// The closure should return a result which returns () if the input is valid and a string error message to be shown if the input is invalid.
...
pub fn inputln<T>(input_str: &str) -> T
where
    T: std::str::FromStr,
    <T as std::str::FromStr>::Err: std::fmt::Debug,
{
    let res = input_internal(input_str, None);
    println!();
    res
}
